...
async def admin_add_cat_name(msg: Message, state: FSMContext):
    ...
    name = msg.text.strip()
    if len(name) > 50:
        await msg.answer("❌ Category name too long (max 50 characters). Please try again:")
        return
    if ":" in name:
        await msg.answer("❌ Category name cannot contain ':' character. Please try again:")
        return
...
async def admin_add_prod_name(msg: Message, state: FSMContext):
    ...
    name = msg.text.strip()
    if len(name) > 100:
        await msg.answer("❌ Product name too long (max 100 characters). Please try again:")
        return
    if ":" in name:
        await msg.answer("❌ Product name cannot contain ':' character. Please try again:")
        return
    await state.update_data(name=name)
    await msg.answer("Enter price in USD (e.g., 49.99):")
    await state.set_state(AddProductStates.price)

@router.message(AddProductStates.price, F.text.regexp(r"^\d+\.?\d{0,2}$"))
