...
class BTCPayClient:
    ...
    async def create_invoice(self, amount_usd: float, telegram_id: int, payment_methods: list[str] | None = None) -> dict:
        ...
        try:
            ...
            raise Exception(f"BTCPay API error: {e.response.status_code} - {e.response.text}")
        except httpx.RequestError as e:
            raise Exception(f"BTCPay connection error: {str(e)}")
        except Exception as e:
            raise Exception(f"Unexpected error creating invoice: {str(e)}")
